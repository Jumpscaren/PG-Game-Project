...
{
	for (auto& buffer_handle : m_buffer_handles[dx12_core->GetCurrentBackBufferIndex()])
	{
		dx12_core->GetBufferManager()->FreeBuffer(buffer_handle);
	}

	for (auto& buffer_view_handle : m_buffer_view_handles[dx12_core->GetCurrentBackBufferIndex()])
	{
		dx12_core->GetBufferManager()->FreeView(buffer_view_handle);
	}

	for (auto& texture_handle : m_texture_handles[dx12_core->GetCurrentBackBufferIndex()])
	{
		dx12_core->GetTextureManager()->FreeTexture(texture_handle);
	}

	for (auto& texture_view_handle : m_texture_view_handles[dx12_core->GetCurrentBackBufferIndex()])
	{
		dx12_core->GetTextureManager()->FreeView(texture_view_handle);
	}

	m_buffer_handles[dx12_core->GetCurrentBackBufferIndex()].clear();
	m_buffer_view_handles[dx12_core->GetCurrentBackBufferIndex()].clear();
	m_texture_handles[dx12_core->GetCurrentBackBufferIndex()].clear();
	m_texture_view_handles[dx12_core->GetCurrentBackBufferIndex()].clear();
}
...
{
	m_buffer_handles[dx12_core->GetCurrentBackBufferIndex()].push_back(buffer_handle);
}

void DX12ResourceDestroyer::FreeBufferView(DX12Core* dx12_core, DX12BufferViewHandle buffer_view_handle)
{
	m_buffer_view_handles[dx12_core->GetCurrentBackBufferIndex()].push_back(buffer_view_handle);
}

void DX12ResourceDestroyer::FreeTexture(DX12Core* dx12_core, DX12TextureHandle texture_handle)
{
	m_texture_handles[dx12_core->GetCurrentBackBufferIndex()].push_back(texture_handle);
}

void DX12ResourceDestroyer::FreeTextureView(DX12Core* dx12_core, DX12TextureViewHandle texture_view_handle)
{
	m_texture_view_handles[dx12_core->GetCurrentBackBufferIndex()].push_back(texture_view_handle);
}
